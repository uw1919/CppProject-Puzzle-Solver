...
// Description: Implements the solver for modified clock problem
// Revisions:
...
public:
	...
	virtual T getStartConfiguration() = 0;
     ...
     *                                      on screen
	 */	
	virtual void print(const T & configuration, ostream & ostr) = 0;
...
list<T> compute(Puzzle<T>  & puzzle)
{
    ...
    T startConfiguration = puzzle.getStartConfiguration();

    //the target configuration
	T target;

	// Queue the starting configuration in possibleConfigurations queue
	possibleConfigurationsQueue.push( startConfiguration );
	// put the start configuration in checkedConfigurations queue
	checkedConfigurations[ startConfiguration ] = startConfiguration;
